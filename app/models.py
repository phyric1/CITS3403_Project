...
class Card(db.Model):
    ...
    rarity = db.Column(db.Enum(CardRarity, validate_strings=True), nullable=False)
    type = db.Column(db.Enum(CardType, validate_strings=True), nullable=False)
...
class DeckCard(db.Model):
    ...
    deck = db.relationship('Deck', backref='deck_cards')
    card = db.relationship('Card') # Cards specified in deck are abstract, and should be resolved to UserCards on game initialisation

    __table_args__ = (
        db.UniqueConstraint('deck_id', 'card_id', name='unique_card_per_deck'),
        db.CheckConstraint('quantity > 0', name='check_quantity_positive'),
    )

    def __repr__(self):
        return f"<DeckCard deck={self.deck_id} card={self.card_id}>"
